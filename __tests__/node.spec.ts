...
describe(resolveNodeScope, () => {
  ...
  it("should use defaults when scope is undefined or 'default'", () => {
    expect(resolveNodeScope()).toStrictEqual({ global: false, patterns: DEFAULT_PATTERNS });
    expect(resolveNodeScope("default")).toStrictEqual({
      global: false,
  ...
  it("should wrap string scope as pattern when not 'default'", () => {
    ...
    expect(result.global).toBe(false);
    expect(result.patterns).toStrictEqual(["custom-pattern"]);
  });

  it("should use array scope directly as patterns", () => {
    const result = resolveNodeScope(["pattern1", "pattern2"]);
    expect(result.global).toBe(false);
    expect(result.patterns).toStrictEqual(["pattern1", "pattern2"]);
  });
  ...
  it("should use custom defaults when provided", () => {
    ...
    expect(result.patterns).toStrictEqual(customDefaults);
  });
});
